...
class DeferredInvestmentCost:
    ...
    def __init__(self, base_precios_path: str, capacity_threshold: float = 0.85, crs=config.PROJECT_CRS, flood_metrics=None):
        self.capacity_threshold = capacity_threshold
        self.base_precios_path = base_precios_path
        self.crs = crs
        self.flood_metrics = flood_metrics
    ...
    def _get_surface_type_from_osm(self, gdf: gpd.GeoDataFrame) -> pd.Series:
        ...
        if not osm_data.empty:
            highways = osm_data[
                osm_data.get('highway', pd.Series()).notna()] if 'highway' in osm_data.columns else gpd.GeoDataFrame()
            amenity = osm_data[
                osm_data.get('amenity', pd.Series()).notna()] if 'amenity' in osm_data.columns else gpd.GeoDataFrame()
            landuse = osm_data[
                osm_data.get('landuse', pd.Series()).notna()] if 'landuse' in osm_data.columns else gpd.GeoDataFrame()
            leisure = osm_data[
                osm_data.get('leisure', pd.Series()).notna()] if 'leisure' in osm_data.columns else gpd.GeoDataFrame()
            natural = osm_data[
                osm_data.get('natural', pd.Series()).notna()] if 'natural' in osm_data.columns else gpd.GeoDataFrame()

            print(
                f"  [OSM] highways={len(highways)}, amenity={len(amenity)}, landuse={len(landuse)}, leisure={len(leisure)}, natural={len(natural)}")
        else:
        ...
        if not highways.empty and 'highway' in highways.columns:
            ...
            if not highways_valid.empty:
                ...
                mask_flexible = hw.isin(['tertiary', 'residential', 'unclassified']) & ~surf.isin(
                    ['unpaved', 'gravel', 'dirt'])
                mask_lastre = (hw.isin(['tertiary', 'residential', 'unclassified']) & surf.isin(
                    ['unpaved', 'gravel', 'dirt'])) | hw.isin(['track', 'service'])
                mask_adoquin = hw.isin(['footway', 'path', 'pedestrian', 'sidewalk']) & surf.isin(
                    ['paving_stones', 'cobblestone', 'sett', 'unhewn_cobblestone'])
                mask_footway_rigido = hw.isin(['footway', 'path', 'pedestrian', 'sidewalk']) & surf.isin(
                    ['concrete', 'paved'])
                mask_footway_flex = hw.isin(
                    ['footway', 'path', 'pedestrian', 'sidewalk']) & ~mask_adoquin & ~mask_footway_rigido
                ...
                centroids = centroids.merge(result[['_orig_idx', 'sup_road']], on='_orig_idx', how='left',
                                            suffixes=('_old', ''))
        ...
        if not landuse.empty and 'landuse' in landuse.columns:
            ...
            if not landuse_valid.empty:
                ...
                if not unclassified.empty:
                    joined = gpd.sjoin(unclassified[['geometry', '_orig_idx']], landuse_valid[['geometry', 'landuse']],
                    ...
                    centroids.loc[centroids['_orig_idx'].isin(update_map.keys()), 'sup_road'] = centroids[
                        '_orig_idx'].map(update_map)
    ...
    def _generate_cost_summary_txt(self, excel_path: str, output_dir: Path, case_name: str, total_cost: float,
            ...
            with open(txt_path, 'w', encoding='utf-8') as f:
                ...
                if 'Material' in gdf.columns and 'Length' in gdf.columns:
                    for mat, grp in gdf.groupby('Material'):
                        ...
                        f.write(
                            f"    - Longitud: {grp['Length'].sum():,.2f} m ({100 * grp['Length'].sum() / gdf['Length'].sum():.1f}%)\n")
                f.write("\n")
    ...
    def _generate_pipe_statistics(self, gdf: gpd.GeoDataFrame, output_dir: Path, case_name: str, total_cost: float):
        ...
        try:
            ...
            if 'L' in gdf.columns:
                ...
                ax3.axvline(lengths.mean(), color='red', linestyle='--', linewidth=2,
                            label=f'Media: {lengths.mean():.1f}m')
                ax3.axvline(lengths.median(), color='orange', linestyle='--', linewidth=2,
                            label=f'Mediana: {lengths.median():.1f}m')
...
"""
    ...
    def run(self, inp_path: str, output_dir: Optional[str] = None) -> float:
        ...
        if not self.failing_pipes.empty:
            ...
            print("\nNo hay tuberías que reemplazar ---")
            self.total_cost = 0.0
...
class AvoidedCostRunner:
    ...
    def __init__(self, output_base: str, base_precios_path: str, base_inp_path: str = None, scenarios_dir: str = None, flood_metrics = None):
    ...
    def run(self, tr_list: List[int]):
        ...
        investment_eval = DeferredInvestmentCost(
            ...
            capacity_threshold=config.CAPACITY_MAX_FOR_AVOIDED_COST,
            flood_metrics=self.flood_metrics
        )
